...
{
    User cristianScript = new User { Id = 3, FirstName = "CristianScript", LastName = "Es el mejor", Username = "cristianScript", PasswordHash = BCryptNet.HashPassword("meGustaProgramar"), Role = Role.Admin };
    using var scope = app.Services.CreateScope();
    var dataContext = scope.ServiceProvider.GetRequiredService<DataContext>();
    dataContext.Users.AddRange(cristianScript);
    dataContext.SaveChanges();
}
